...
- **Multi-PC Phase 2 — DONE (2026-06-04).** The Windows agent gains two capabilities and "on the \<pc\>" targeting is generalized beyond open_app. **`pc-agent/capabilities/media.js`** (`makeMedia`) sends Windows **media/volume virtual keys** via PowerShell `keybd_event` (down+up; VKs play_pause `0xB3` / next `0xB0` / prev `0xB1` / volume_up `0xAF` / volume_down `0xAE` / mute `0xAD`) — controls whatever app has media focus, like the keyboard media keys. **`pc-agent/capabilities/shell.js`** (`makeShell`) runs a **PowerShell** command fire-and-forget; the agent runs whatever an authed caller sends — **the safety gate is on the orchestrator**. `pc-agent/index.js` now boots `[apps, media, shell]` (so `/health` lists all three). Orchestrator: `intent/pc.js` now strips a trailing **"on (the) \<known-pc\>" once from the whole text** (`stripMachine`, replacing the open_app-only `splitMachine`) and attaches `machine` to **open_app / media / shell** intents (window/search/split stay local — note: a search query ending exactly in "on the \<pcname\>" gets truncated, an accepted trade-off). `router.js` routes remote **media transport ops** (the 6 above) to `agentClient.run(...,{capability:'media',action:op,params:{}})` via a shared **`remoteSpeak(r,machine)`** helper (now exported); `play_music`/`stop_music`/`set_volume` with a machine → polite "I can't do that on the \<pc\> yet." The confirm-gated **shell** flow in `server.js makePipeline` now, when the intent carries a machine, stores a **literal** command (no local recipe lookup) + the machine on `pending`, prompts "Should I run \<cmd\> on the \<pc\>? Say confirm to run.", and on a fresh spoken **"confirm"** calls `agentClient.run(...,{capability:'shell',action:'run',params:{command}})` (reusing `remoteSpeak`, so a reachable-but-failed command surfaces its detail instead of a false "couldn't reach"). **Invariant preserved: nothing runs, local or remote, without a fresh spoken "confirm".** Out of scope: remote play-a-song, a system/power capability, capability-driven vocab from `/health`, multiple agents in one command. 293 node tests green. Merged to `main`. **Live setup (needs the Windows PC):** pull/update the agent, restart `node pc-agent/index.js` (`/health` → apps, media, shell), then "pause on the desktop", "volume up on the desktop", "run notepad on the desktop" → "confirm".

- **Multi-PC Phase 1 — DONE (2026-06-01).** "open \<app\> on the \<pc\>" launches an app on a **remote Windows PC** running a tiny agent; local "open \<app\>" is unchanged. **`pc-agent/`** is a dependency-free Node `http` server (zero deps — `node index.js`): `GET /health` + bearer-gated `POST /run {capability,action,params}`, capability loader, `apps.open` (Windows `start`); README has Windows setup. Orchestrator: `config.parsePcAgents` reads **`PC_AGENTS=name=url,...`** → seeded as `pc_agent` rows (`registry.getPcAgents/getPcAgent`); their names go in `vocab.pcNames`; `intent/pc.js` strips a trailing **"on (the) \<known-pc\>"** into `intent.machine`; `router.js` open_app with a machine → **`devices/pc-agent-client.js`** `run()` (POST /run, Bearer `PC_AGENT_TOKEN`, 8s timeout) and surfaces the agent's detail (network failure → "couldn't reach", action failure → its message). 270 node tests green. Merged to `main`. **Later phases:** media/shell/system capabilities, remote confirm-gate, multiple agents. **Live setup (needs the Windows PC):** install Node on Windows, set `PC_AGENT_TOKEN`, `node pc-agent/index.js`, open the firewall port; orchestrator `.env`: same `PC_AGENT_TOKEN` + `PC_AGENTS=desktop=http://<win-ip>:7000`; then "open notepad on the desktop".

- **Phone vision source — DONE (2026-06-01).** The webcam source is gone (no webcam on this box); vision now has **phone + screen**. `capture.phone()` HTTP-fetches an **IP Webcam** snapshot (`PHONE_CAMERA_URL`, e.g. `http://192.168.0.187:8080/photo.jpg`) → base64 (MIME stripped of any `; charset`), feeding the existing multimodal-Gemini vision pipeline. **Phone is the default** for "look at this" / "what is this" / "look at what I'm doing" / "look through my phone"; **screen** only on "look at my screen"/"display". Graceful: no URL → "I don't have a phone camera set up, sir."; unreachable → "I couldn't reach your phone's camera." `capture.camera()` (ffmpeg/v4l2) removed; `makeVision({phone,screen,describe})`. `PHONE_CAMERA_URL` set in local `.env` (gitignored); placeholder in `.env.example`. 248 node tests green. Merged to `main`. **Live use:** run an IP-Webcam app on the phone, confirm the snapshot URL opens in a browser, restart via `./run-jarvis.sh`.
