...
test('phone source: captures then describes', async () => {
  ...
  // describe() returns { ok, speak } (see vision-answer.js) — look() must unwrap it.
  const describe = async (q, data, mime) => { seen.push({ q, data, mime }); return { ok: true, speak: 'a mug' }; };
  const v = makeVision({ phone, screen, describe });
  const r = await v.look({ source: 'phone', query: 'what is this' });
  assert.equal(r.ok, true);
  assert.equal(typeof r.speak, 'string', 'speak must be the plain string, not the describe() object');
...
test('default (no source) uses the phone capturer', async () => {
  ...
  const v = makeVision({ phone, screen, describe: async () => ({ ok: true, speak: 'd' }) });
...
test('capture failure short-circuits without describing', async () => {
  ...
  const describe = async () => { described = true; return { ok: true, speak: 'x' }; };
